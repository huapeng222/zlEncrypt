...
+ (SecKeyRef)addPublicKey:(NSString *)key{
    ...
    data = [ZLRsaUtil stripPublicKeyHeader:data];
    if(!data){
        return nil;
    }
...
+ (NSString *)encryptString:(NSString *)str publicKey:(NSString *)pubKey{
    ...
    for (int i = 0; i < totalLength; i += 117) {
        int length = totalLength - i;
        if (length > 117) {
            length = 117;
        }
        NSData *d = [data subdataWithRange:NSMakeRange(i, length)];
        [resultData appendData:[ZLRsaUtil encryptData:d publicKey:pubKey]];
    }
    NSString *ret = base64_encode_data(resultData);
    return ret;
}
...
+ (NSData *)encryptData:(NSData *)data publicKey:(NSString *)pubKey{
    ...
    SecKeyRef keyRef = [ZLRsaUtil addPublicKey:pubKey];
    if(!keyRef){
        return nil;
    }
...
+ (NSString *)decryptString:(NSString *)str publicKey:(NSString *)pubKey{
    NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
    data = [ZLRsaUtil decryptData:data publicKey:pubKey];
    NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    return ret;
}
